...
class tc_simple_import : public test_case
{
public:
	tc_simple_import( const std::string& path ) : test_case( path ) { }
	...
	void on_case() override
	{
		...
		{
			if( this->m_interp->get_vdr_from_env( VDR_0, this->m_test_env_name ).vdr_fl == 3.141f && this->m_interp->
				get_vdr_from_env( VDR_1, this->m_test_env_name ).vdr64 == 1347 )
			{
