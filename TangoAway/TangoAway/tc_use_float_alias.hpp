...
class tc_use_float_alias : public test_case
{
public:
	tc_use_float_alias(const std::string& path) : test_case(path) { }
	...
	void on_case() override
	{
		...
		{
			if (this->m_interp->get_vdr_from_env(VDR_0, this->m_test_env_name).vdr_fl == 1.f)
			{
