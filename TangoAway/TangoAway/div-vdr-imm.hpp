...
class inst_div_vdr_imm : public instruction
{
public:
	inst_div_vdr_imm( const std::string& type, const EVdrIdentifier dest_ident, const std::string& src )
	{
	...
	void evaluate(std::shared_ptr< environment > env ) override
	{
		...
		if( m_datatype == "byte" )
		{
			env->write_to_vdr< uint8_t >( this->m_destination,
			                              env->read_from_vdr( this->m_destination )->vdr_.vdr8 / num
			);
		...
		if( m_datatype == "word" )
		{
			env->write_to_vdr< uint16_t >( this->m_destination,
			                               env->read_from_vdr( this->m_destination )->vdr_.vdr16 / num
			);
		...
		if( m_datatype == "dword" )
		{
			env->write_to_vdr< uint32_t >( this->m_destination,
			                               env->read_from_vdr( this->m_destination )->vdr_.vdr32 / num
			);
		...
		if( m_datatype == "qword" )
		{
			env->write_to_vdr< uint64_t >( this->m_destination,
			                               env->read_from_vdr( this->m_destination )->vdr_.vdr64 / num
			);
		...
		env->write_to_vdr< float >( this->m_destination, env->read_from_vdr( this->m_destination )->vdr_.vdr_fl / fl );
